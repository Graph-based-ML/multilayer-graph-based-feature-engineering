...
for alpha in "${alphas[@]}"; do
    ...
    screen -S "${SCREEN_NAME:0:15}" -dm bash -c "
      source $VENV_PATH/bin/activate
      {
        ...
        parallel ::: \
          \"python3 -m scripts.03_graph_construction --graph_with_class --metric='accuracy' --cwd=$cwd --dataset_folder=$param1 --alpha=$alpha --turn=2\" \
          \"python3 -m scripts.03_graph_construction --graph_with_class --metric='f1-score' --cwd=$cwd --dataset_folder=$param1 --alpha=$alpha --turn=2\" \
          \"python3 -m scripts.03_graph_construction --metric='accuracy' --cwd=$cwd --dataset_folder=$param1 --alpha=$alpha --turn=2\" \
          \"python3 -m scripts.03_graph_construction --metric='f1-score' --cwd=$cwd --dataset_folder=$param1 --alpha=$alpha --turn=2\"

        parallel ::: \
          \"python3 -m scripts.04_model_training --metric='accuracy' --cwd=$cwd --dataset_folder=$param1 --alpha=$alpha --turn=2\" \
          \"python3 -m scripts.04_model_training --metric='f1-score' --cwd=$cwd --dataset_folder=$param1 --alpha=$alpha --turn=2\" \
